// コスト消費
buff.cons_cost = [
    {
        name: "ウルカノ/花嫁", rarity: "黒", cl: "鍛冶職人", AW: "覚1", skill: "覚醒"
        , stats: { atk: "×1.4", def: "×1.4" }
        , target: [
            { cl: [ "王子" ] }
            , { sex: [ "女性" ] }
        ]
        , note: [ "最大値(コスト消費：150)", "永続" ]
    }
    , {
        name: "ウルカノ/花嫁", rarity: "黒", cl: "鍛冶職人", AW: "覚2a", skill: "覚醒"
        , stats: { atk: "×1.5", def: "×1.4" }
        , target: [
            { cl: [ "王子" ] }
            , { sex: [ "女性" ] }
        ]
        , note: [ "最大値(コスト消費：150)", "永続" ]
    }
    , {
        name: "ウルカノ/花嫁", rarity: "黒", cl: "鍛冶職人", AW: "覚2b", skill: "覚醒"
        , stats: { atk: "×1.5", def: "×1.5" }
        , target: [
            { cl: [ "王子" ] }
            , { sex: [ "女性" ] }
        ]
        , note: [ "最大値(コスト消費：150)", "永続" ]
    }
];

/*

    , {
        name: "", rarity: "", cl: "", AW: "覚醒", skill: ""
        , stats: { : "" }
        , target: { : [ "" ] }
        , target: [
            { : [ "" ] }
            , { : [ "" ] }
        ]
    }
    
    黒 白 青 金 ちび 銀 銅 鉄 トークン
    
    未 覚醒 覚1 覚2a 覚2b
    
    常時 非 通常 覚醒
    
    hp atk def mr range cost wt ct dur atkCd
    
    rege: [ "/秒", "/f" ]
    
    poison: "-100%", abn: "-100%"
    
    eva nul redep
    
    draw
    ○
    (○)
    
    other
    
    depType 近接 遠距離 遠近距離
    
    sex 男性 女性
    
    home 白の帝国 砂漠の国 東の国 パルス王国
    
    race 聖霊 人間 獣人 竜人 エルフ系 ドワーフ ヴァンパイア
    
    デーモン系 天使 妖怪 仙人 オーク ゴブリン 鳥人 魚人
    
    神 天界人 ねんどろいど
    
    season お正月 温泉 バレンタイン 学園 エッグハント
    
    ジューンブライド サマー ハロウィン クリスマス
    
    others 英傑 ちび 弓兵 魔術師 アンデッド 機械 騎兵 重装 聖職者 魔界 ドラゴン ＜七つの大罪＞
    
    全プリンセス系 GG持ち HP回復不可 魔界適応 深海適応 天界適応
    
    year 年
    
    note 範囲 分類未確認
    
    正月 水着 浴衣 バニー 花嫁

*/